...
import slmp.moduel.view.hintFrame.OO_InsertJFrame;

import javax.swing.JOptionPane;

public class out_orderJPanel extends JPanel {
	 ...
	 * Create the panel.
	 */
	public out_orderJPanel(String id) {
		this.setUserID(id);
		
		StockOrderSer out_order = new StockOrderSer();
		...
		List list = null;
		Object[] title = {"订单号", "商品名称", "出库数量", "所属分类", "所属仓库", "经手人"};
		
		try {
			list = out_order.selectAllStockOutput();
		} catch (Exception e) {
	...
	private class CRUDLisener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource() == insert) {
				OO_InsertJFrame insertFrame = new OO_InsertJFrame();
